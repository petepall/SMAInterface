...
function generateSignature(secret, method, service, timestamp, identifier) {
	return encodeURIComponent(crypto.createHmac('sha1', secret)
...
const plantPath = `/services/plantlist/100/${identify}?timestamp=${timestamp}&signature-method=auth&signature-version=100&signature=${generateSignature(key, method, service, timestamp, identify)}`;
...
const dataPath = `/services/data/100/${plantoid}/overview-day-fifteen-total/${date}?culture=en-gb&identifier=${identify}&timestamp=${timestamp}&signature-method=auth&signature-version=100&signature=${generateSignature(key, method, service, timestamp, identify)}`;
...
const deletePath = `/services/authentication/100/${identify}?timestamp=${timestamp}&signature-method=auth&signature-version=100&signature=${generateSignature(key, method, service, timestamp, identify)}`;
